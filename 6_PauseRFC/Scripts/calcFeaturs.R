...
# Use: ./calcFeatures.R mut real/shuffled
...
for (i in 1:nrow(data)) {
	...
	temp = read.table(pipe(paste(bashCommand, '../0_Annotations/ChromatinFeatures/', DATA_TYPE, '.bed ', MUT, '_', TYPE, '.tmpCoord.txt', sep='')), sep="\t", header=FALSE, col.names=c("Chr", "Start", "End", "Value"), colClasses=c('character', rep('numeric', 3)))
    ...
    temp = read.table(pipe(paste('closestBed -t first -a ', MUT, '_', TYPE, '.tmpCoord.txt -b ../0_Annotations/ChromatinFeatures/nucCenters.bed', sep='')), sep="\t", header=FALSE, col.names=c("PauseChr", "PauseStart", "PauseEnd", "NucChr", "NucStart", "NucEnd"), colClasses=c('character', 'numeric', 'numeric', 'character', 'numeric', 'numeric'))
    data$NucCenter[i] = abs(temp$NucStart[1] - temp$PauseStart[1])

    # Percent into gene body
    if (data$Strand[i] == "+") { 
       temp = read.table(pipe(paste('intersectBed -wb -a ', MUT, '_', TYPE, '.tmpCoord.txt -b ../0_Annotations/Genes/genes_all_buffer200.pos.bed', sep='')), sep="\t", header=FALSE, col.names=c("PauseChr", "PauseStart", "PauseEnd", "GeneChr", "GeneStart", "GeneEnd", "GeneName", "GeneValue", "GeneStrand"), colClasses=c('character', 'numeric', 'numeric', 'character', 'numeric', 'numeric', 'character', 'character', 'character')) 
       data$GenePerc[i] = round(((temp$PauseStart[1] + 200 - temp$GeneStart[1]) / (temp$GeneEnd[1] - temp$GeneStart[1] + 400)) * 100, 2)
       data$DistFromTSS[i] = temp$PauseStart[1]  - (temp$GeneStart[1] + 200)
       data$DistFrompA[i] = temp$GeneEnd[1] + 200 - temp$PauseStart[1]
    } else {
       temp = read.table(pipe(paste('intersectBed -wb -a ', MUT, '_', TYPE, '.tmpCoord.txt -b ../0_Annotations/ChromatinFeatures/genes_all_buffer200.neg.bed', sep='')), sep="\t", header=FALSE, col.names=c("PauseChr", "PauseStart", "PauseEnd", "GeneChr", "GeneStart", "GeneEnd", "GeneName", "GeneValue", "GeneStrand"), colClasses=c('character', 'numeric', 'numeric', 'character', 'numeric', 'numeric', 'character', 'character', 'character'))
       data$GenePerc[i] = 100 - round(((temp$PauseStart[1] + 200 - temp$GeneStart[1]) / (temp$GeneEnd[1] - temp$GeneStart[1] + 400))  * 100, 2)
       data$DistFromTSS[i] = temp$GeneEnd[1] + 200 - temp$PauseStart[1]
       data$DistFrompA[i] = temp$GeneEnd[1] + 200 - temp$PauseStart[1]
    }
