...
do
    ...
    awk -F "\t" 'BEGIN {OFS="\t"} ($4 > 0) {print $1, $2, $3, $4*($3-$2)}' $posSignal | mapBed -a $posGenes -g $genome -b - -c 4 -o sum | awk -F "\t" 'BEGIN {OFS="\t"} ($7 != ".") {print $0, $7/($3-$2)}' | awk -v t=$t -F "\t" 'BEGIN {OFS="\t"} ($8 > t) {print $1, $2, $3, $4, $5, $6, $7, $8}' > ${mut}-1.ReadscovThreshold${t}.pos.bed
    awk -F "\t" 'BEGIN {OFS="\t"} ($4 > 0) {print $1, $2, $3, $4*($3-$2)}' $negSignal | mapBed -a $negGenes -g $genome -b - -c 4 -o sum | awk -F "\t" 'BEGIN {OFS="\t"} ($7 != ".") {print $0, $7/($3-$2)}' | awk -v t=$t -F "\t" 'BEGIN {OFS="\t"} ($8 > t) {print $1, $2, $3, $4, $5, $6, $7, $8}' > ${mut}-1.ReadscovThreshold${t}.neg.bed

    # Repeat for replicate 2
    posSignal=../1_AlignData/${mut}-2_stt_pos_sort.bedGraph 
    negSignal=../1_AlignData/${mut}-2_stt_neg_sort.bedGraph 

    awk -F "\t" 'BEGIN {OFS="\t"} ($4 > 0) {print $1, $2, $3, $4*($3-$2)}' $posSignal | mapBed -a $posGenes -g $genome -b - -c 4 -o sum | awk -F "\t" 'BEGIN {OFS="\t"} ($7 != ".") {print $0, $7/($3-$2)}' | awk -v t=$t -F "\t" 'BEGIN {OFS="\t"} ($8 > t) {print $1, $2, $3, $4, $5, $6, $7, $8}' > ${mut}-2.ReadsovThreshold${t}.pos.bed
    awk -F "\t" 'BEGIN {OFS="\t"} ($4 > 0) {print $1, $2, $3, $4*($3-$2)}' $negSignal | mapBed -a $negGenes -g $genome -b - -c 4 -o sum | awk -F "\t" 'BEGIN {OFS="\t"} ($7 != ".") {print $0, $7/($3-$2)}' | awk -v t=$t -F "\t" 'BEGIN {OFS="\t"} ($8 > t) {print $1, $2, $3, $4, $5, $6, $7, $8}' > ${mut}-2.ReadscovThreshold${t}.neg.bed

    # Count number of reads in possible pauses for both replicates
    reads1=`cat ${mut}-1.ReadscovThreshold${t}.???.bed | awk -F "\t" '{sum += $7} END {print sum}'`
    reads2=`cat ${mut}-2.ReadscovThreshold${t}.???.bed | awk -F "\t" '{sum += $7} END {print sum}'`
