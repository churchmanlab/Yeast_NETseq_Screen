...
for mut in $muts
do
sbatch -o logs/makeMatrix_${mut}.log -e logs/makeMatrix_${mut}.err ./Scripts/makeDESeqMatrix_MC.sh ${mut}-1 ${mut}-2 $mut
done


module load bedops/2.4.30

pos_gene_file="../0_Annotations/Genes/genes_polII.noOver.coding_pos.bed"
neg_gene_file="../0_Annotations/Genes/genes_polII.noOver.coding_neg.bed"
...
# wt1_pos_file="../1_AlignData/wt-1_stt_pos_sort.bedGraph" # "../1_AlignData/wt-1_cov_pos.bedGraph"
# wt1_neg_file="../1_AlignData/wt-1_stt_neg_sort.bedGraph"
# wt2_pos_file="../1_AlignData/wt-2_stt_pos_sort.bedGraph"
# wt2_neg_file="../1_AlignData/wt-2_stt_neg_sort.bedGraph"
# wt3_pos_file="../1_AlignData/wt-3_stt_pos_sort.bedGraph"
# wt3_neg_file="../1_AlignData/wt-3_stt_neg_sort.bedGraph"
# wt4_pos_file="../1_AlignData/wt-4_stt_pos_sort.bedGraph"
# wt4_neg_file="../1_AlignData/wt-4_stt_neg_sort.bedGraph"


# Overlap individually with gene files (resort here, which sorts wrongly, to match wrongly sorted gene file)
# sortBed -i $wt1_pos_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $pos_gene_file - > tmp.wt1.pos.bed
# sortBed -i $wt1_neg_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $neg_gene_file - > tmp.wt1.neg.bed
# sortBed -i $wt2_pos_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $pos_gene_file - > tmp.wt2.pos.bed
# sortBed -i $wt2_neg_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $neg_gene_file - > tmp.wt2.neg.bed
# sortBed -i $wt3_pos_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $pos_gene_file - > tmp.wt3.pos.bed
# sortBed -i $wt3_neg_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $neg_gene_file - > tmp.wt3.neg.bed
# sortBed -i $wt4_pos_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $pos_gene_file - > tmp.wt4.pos.bed
# sortBed -i $wt4_neg_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $neg_gene_file - > tmp.wt4.neg.bed
...
if [ "$#" -eq 3 ]; then
    ...
    # Get input files
    mut1_pos_file="../1_AlignData/${mut1}_stt_pos_sort.bedGraph"
    mut1_neg_file="../1_AlignData/${mut1}_stt_neg_sort.bedGraph"
    mut2_pos_file="../1_AlignData/${mut2}_stt_pos_sort.bedGraph"
    mut2_neg_file="../1_AlignData/${mut2}_stt_neg_sort.bedGraph"
    
    # Overlap individually with gene files
    sortBed -i $mut1_pos_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $pos_gene_file - > tmp.${mut1}.pos.bed
    sortBed -i $mut1_neg_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $neg_gene_file - > tmp.${mut1}.neg.bed
    sortBed -i $mut2_pos_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $pos_gene_file - > tmp.${mut2}.pos.bed
    sortBed -i $mut2_neg_file | awk 'BEGIN {OFS="\t"} {print $1, $2, $3, ".", $4}' - | bedmap --echo --delim "\t" --sum $neg_gene_file - > tmp.${mut2}.neg.bed
