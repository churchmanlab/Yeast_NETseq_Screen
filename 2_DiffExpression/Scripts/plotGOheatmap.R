...
mat <- read.table("GOoverlaps.txt", sep = "\t", stringsAsFactors = FALSE)
...
write.table(df, "plotGO_output.txt", quote = F, sep = "\t", row.names = T, col.names = T)
...
ggsave(file="GOheatmap.svg", plot=p1, width=5, height=7)
